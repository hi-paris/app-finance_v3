...
def Q1_compute_holdingperiod_returns(df_risky):
    asset1_returns = (df_risky["Price"][1:].to_numpy()
                      - df_risky["Price"][:-1].to_numpy()
                      + df_risky["Dividends"].to_numpy()[1:]) / df_risky["Price"][:-1].to_numpy()
...
def Q3_return_std_portfolios(asset1_returns, weight_risky_portfolios, risky_asset, riskfree_std, riskfree_exp_returns):
    ...
    expected_returns_portfolios = np.array(
        [w*asset_expected_return + (1-w)*riskfree_exp_returns for w in weight_risky_portfolios]
    )
    std_portfolios = np.array(
        [(w*asset_std_dev)**2 + ((1-w)*riskfree_std)**2 for w in weight_risky_portfolios]
    )
    std_portfolios = np.sqrt(std_portfolios)

    # Portfolio dataframe to plot
    df_portfolios = pd.DataFrame({
        f"{risky_asset}": np.round(weight_risky_portfolios, 2),
...
def lab1_code(select_teacher, select_code, student_ids, simple_mode: bool = False):
    startdate = datetime.now()
    default_text = ""
    list_risky_assets = ["ACMTA", "ACU", "AIR", "ASA", "BKTI", "CECO", "PRG"]

    #################################### SIDEBAR ##################################
    # On garde ce sélecteur même en simple_mode
    ...
    # Holding-period returns (masquer upload en simple_mode)
    st.write(f"**Compute the holding-period returns**")
    answer_1_Q1_1 = ""
    if simple_mode:
        upload_expected_return = None
    else:
        ...
        if upload_expected_return is not None:
            ...
            _ = pd.read_excel(upload_expected_return)

    st.markdown("  ")
    st.markdown("  ")

    # Expected returns (inputs conservés)
    ...
    if st.checkbox('**Solutions - Question 1** ✅'):
        ...
        st.success("**Instruction**: Compute the expected return and standard deviation of the holding-period returns.")
        st.markdown(f"""
    ...
    st.markdown(
        '''<p style="font-size: 22px;">Assume that you have a capital of 1000 EUR that you fully invest in a portfolio. <b>Combine two assets</b> (one risky and one risk-free asset) into a <b>portfolio</b>. Next, <b>compute the expected returns</b> and <b>standard deviation</b> of the portfolio.</p>''',
        unsafe_allow_html=True
    )
    ...
    # Expected return / Std (user inputs)
    ...
    st.markdown(
        '''<p style="font-size: 22px;"> Using Excel, <b> construct portfolios </b> that contain x% of the risky asset and (1-x)% of the risk-free asset, with x varying between 0 and 100% with 1% increments.
    ...
    # Préparation (identique), mais la soumission et les messages sont cachés en simple_mode
    list_answer = [
        answer_1_Q1_1, answer_1_Q1_2, answer_1_Q1_3,
        answer_1_Q2_1, answer_1_Q2_2, answer_1_Q2_3, answer_1_Q2_4,
        answer_1_Q3_1, answer_1_Q3_2,
        answer_1_Q4_1, answer_1_Q4_2, answer_1_Q4_3, answer_1_Q4_4,
        answer_1_Q5_1, answer_1_Q5_2,
        answer_1_Q6_1, answer_1_Q6_2, answer_1_Q6_3, answer_1_Q6_4,
    ]
    answers = [x for x in list_answer if x not in [np.nan, "", None]]
    count = len(answers)

    list_answers_ = [
        select_teacher, select_code, student_ids,
        1, risky_asset, str(datetime.now().date()),
        count, f"{round((count/len(list_answer))*100,1)}%",
        *list_answer
    ]

    # ====== BLOC DE SOUMISSION MASQUÉ EN SIMPLE_MODE ======
    if not simple_mode:
        ...
        st.sidebar.markdown("""**Note**: The green text box confirming your submission might not appear after clicking the submit button. 
                                This doesn't mean your submission wasn't registered, as it is just an app display issue.""",
                             unsafe_allow_html=True)
