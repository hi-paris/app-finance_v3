...
def two_stocks_charts(df_asset_ex2_merge, risky_asset1_ex2, risky_asset2_ex2):
    df_asset_ex2_graph1 = df_asset_ex2_merge.drop(
        columns=[f"Dividends ({risky_asset1_ex2})", f"Dividends ({risky_asset2_ex2})"]
    ).melt(id_vars=["Year"])
...
def Q2_return_std_portfolios_risky(asset1_returns, asset2_returns, weight_portfolios, risky_asset1_ex2, risky_asset2_ex2):
    ...
    expected_returns_portfolios = np.array(
        [w*asset1_expected_return + (1-w)*asset2_expected_return for w in weight_portfolios]
    )
    std_portfolios = np.array(
        [(w*asset1_std_dev)**2 + ((1-w)*asset2_std_dev)**2 + 2*w*(1-w)*asset12_corr*asset1_std_dev*asset2_std_dev
         for w in weight_portfolios]
    )
    std_portfolios = np.sqrt(std_portfolios)

    df_exp_std_return_portfolios = pd.DataFrame({
        risky_asset1_ex2: np.round(weight_portfolios, 2),
...
def Q3_efficient_portfolios(df):
    ...
    min_std_q3_exp = df_ex2_q3_efficient.loc[
        df_ex2_q3_efficient["Standard deviation"] == min_std_q3, "Expected return"
    ].idxmax()
...
def Q5_tangency_portfolio(df, risky_asset1_ex2, risky_asset2_ex2):
    ...
    max_sharpe_weight1 = max_sharpe_ratio_row[risky_asset1_ex2]
    max_sharpe_weight2 = max_sharpe_ratio_row[risky_asset2_ex2]
    max_sharpe = max_sharpe_ratio_row["Sharpe Ratio"]
...
def Q6_compute_weights():
    ...
    for wp in weight_portfoliosR:
        for w1 in weight_portfoliosR:
            ...
            weight_riskfree_full.append(1 - wp)

            weight_risk1_portfolioR.append(w1 / wp if wp != 0 else 0)
            weight_risk2_portfolioR.append((wp - w1) / wp if wp != 0 else 0)

    return (weight_risk1_full, weight_risk2_full, weight_riskportfolio,
            weight_riskfree_full, weight_risk1_portfolioR, weight_risk2_portfolioR)
...
def Q6_portfolio_tworisky_riskfree(asset1_returns, asset2_returns, risky_asset1_ex2, risky_asset2_ex2,
    ...
    (weight_risk1_full, weight_risk2_full, weight_riskportfolio,
     weight_riskfree, weight_risk1_portfolioR, weight_risk2_portfolioR) = Q6_compute_weights()

    df_full_portfolio = pd.DataFrame({
        risky_asset1_ex2: np.round(weight_risk1_full, 2),
    ...
    expected_returns_portfolios = np.array([
        w1*asset1_expected_return + w2*asset2_expected_return + w3*0.02
        for w1, w2, w3 in zip(weight_risk1_full, weight_risk2_full, weight_riskfree)
    ])
    std_portfolios = np.array([
        (w1*asset1_std_dev)**2 + (w2*asset2_std_dev)**2 + 2*w1*w2*asset12_corr*asset1_std_dev*asset2_std_dev
        for w1, w2 in zip(weight_risk1_full, weight_risk2_full)
    ])
    std_portfolios = np.sqrt(std_portfolios)
    ...
    df_efficient_portfolios = df_full_portfolio.loc[
        (df_full_portfolio[f"{risky_asset1_ex2} (in risky portfolio)"] == max_sharpe_weight1) &
        (df_full_portfolio[f"{risky_asset2_ex2} (in risky portfolio)"] == max_sharpe_weight2)
    ].drop(columns=[f"{risky_asset1_ex2} (in risky portfolio)", f"{risky_asset2_ex2} (in risky portfolio)"])
...
def lab2_code(select_teacher, select_code, student_ids, simple_mode: bool = False):
    ...
    if risky_asset1_ex2:
        risky_asset2_ex2 = st.sidebar.selectbox(
            'Select the second risky stock ➡️',
            [elem for elem in list_risky_assets if elem != risky_asset1_ex2]
        )
    ...
    st.markdown(
        '''<p style="font-size: 22px;"> Download prices for two risky stocks. <b>Compute their realized returns</b>.  
...
- **Standard deviation**: {asset_std_dev}
    ...
    st.markdown(
        '''<p style="font-size: 22px;"> Compose different <b>portfolios of two risky assets</b> by investing in one risky asset x% of your wealth and in the other asset (1-x)%.
    ...
    df_exp_std_return_portfolios = Q2_return_std_portfolios_risky(
        asset1_returns, asset2_returns, weight_portfolios, risky_asset1_ex2, risky_asset2_ex2
    )
    ...
    st.markdown(
        '''<p style="font-size: 22px;"> Indicate the set of <b>feasible portfolios</b> and the set of <b>efficient portfolios</b>. Next, <b>draw a graph in which you represent the portfolios</b>, that is, the sigma-expected return pairs, you obtain with different combinations of the two risky assets.''',
        unsafe_allow_html=True
    )
    st.markdown(" ")
    ...
    st.markdown(
        '''<p style="font-size: 22px;"> Assume that you cannot short-sell any of the risky assets. 
    ...
    if st.checkbox(f'**Solutions - Question 4** ✅'):
        df_exp_std_return_portfolios_q4 = df_exp_std_return_portfolios.loc[
            (df_exp_std_return_portfolios[risky_asset1_ex2] >= 0) &
            (df_exp_std_return_portfolios[risky_asset2_ex2] >= 0)
        ]
        st.success("**Instruction**: *What is the set of feasible portfolios ?*")
    ...
    st.markdown(
        '''<p style="font-size: 22px;"> Assume that you also have a risk-free asset with a rate of return of 2% per annum. Assume that you can short-sell any asset.
    ...
    df_exp_std_return_portfolios, max_sharpe, max_sharpe_weight1, max_sharpe_weight2 = Q5_tangency_portfolio(
        df_exp_std_return_portfolios, risky_asset1_ex2, risky_asset2_ex2
    )

    if st.checkbox(f'**Solutions - Question 5** ✅'):
        st.markdown(
            f"""The tangency portfolio is the portfolio where you invest **{max_sharpe_weight1}** in {risky_asset1_ex2} and **{max_sharpe_weight2}** in {risky_asset2_ex2}, with a sharpe ratio of **{max_sharpe}**. <br>
                    *The tangency portfolio is highlighted in green in the dataset*""",
            unsafe_allow_html=True
        )
    ...
    st.markdown(
        '''<p style="font-size: 22px;">Indicate the <b>set of efficient portfolios</b> that you can achieve if you invest in two risky assets and one risk-free asset.''',
        unsafe_allow_html=True
    )
    ...
    if st.checkbox(f'**Solutions - Question 6** ✅'):
        st.markdown(
            f"""The efficient portfolios are the portfolios with a **combination of the risk-free asset and the tangency portfolio of Question 5.** <br>
                    *The weights for {risky_asset1_ex2} and {risky_asset2_ex2} were computed based on the overall portfolio, not on the risky portfolio (the portfolio with only risky assets).*""",
            unsafe_allow_html=True
        )
        df_efficient_portfolios = Q6_portfolio_tworisky_riskfree(
            asset1_returns, asset2_returns, risky_asset1_ex2, risky_asset2_ex2, max_sharpe_weight1, max_sharpe_weight2
        )
        st.markdown(" ")
